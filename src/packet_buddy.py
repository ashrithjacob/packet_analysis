...
class ChatWithPCAP:
    ...
    def initialize_llm_chains(self):
        ...
        def create_qa_chain(model, alias):
            llm = Ollama(
                model=model, base_url=f"http://localhost:80/api/{alias}/generate"
            )
    ...
    def chat(self, question):
        ...
        system_text = st.session_state.get(
            "priming_text", returnSystemText(retrieved_text)
        )
        ...
        response_placeholders["answer"] =self.reasoning_logic(lm=lm, context=full_prompt, question=question)
        return response_placeholders
...
def model_selection():
    ...
    for idx, model in enumerate(all_models):
        ...
        with col:
            st.session_state.selected_models[model] = st.checkbox(
                model, value=st.session_state.selected_models[model], key=model
            )
